...
export default function CategoryPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditing ? `${API_URL}/api/categories/${formData.id}` : `${API_URL}/api/categories`;
  ...
  const handleStatusToggle = async (category) => {
    try {
      ...
      Object.keys(category).forEach((key) => {
        if (key !== 'status' && key !== 'image' && category[key] != null) {
          formDataToSend.append(key, category[key]);
        }
      });
  ...
  return (
    <div className="container w-full mx-auto p-4 sm:p-6 bg-white rounded-lg shadow-lg">
      ...
      {viewMode === 'form' && (
        <div className="bg-gray-50 p-4 sm:p-6 rounded-lg shadow-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-6" encType="multipart/form-data">
            <input type="hidden" name="image" value={imagePreview || ''} readOnly />
                  ...
                  {isEditing && imagePreview && (
                    <div>
                      <p className="text-sm text-gray-600">
                        Current Image: {imagePreview.split('/Uploads/category-image/').pop() || 'No image'}
                      </p>
                      <Image
                        src={imagePreview}
                        alt="Preview"
      ...
      {viewMode === 'table' && (
        <div className="bg-gray-50 p-4 sm:p-6 rounded-lg shadow-md">
          <h2 className="text-xl sm:text-2xl font-semibold mb-4 text-gray-800">Categories List</h2>
          {loading ? (
                ...
                <tbody>
                  {categories.map((category) => (
                    <tr key={category.id} className="hover:bg-gray-50">
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.id}</td>
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.name}</td>
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.title || '-'}</td>
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.sub_title || '-'}</td>
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.slug}</td>
                      ...
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">{category.seo_title || '-'}</td>
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">
                        <div className="truncate max-w-[150px] sm:max-w-[200px]">
                          {category.seo_description?.substring(0, 50)}
                          {category.seo_description?.length > 50 ? '...' : ''}
                        </div>
                      </td>
                      ...
                      <td className="border border-gray-200 p-2 sm:p-3 text-black">
                        <div className="flex items-center">
                          <span className="mr-2 text-sm">{category.status === 1 ? 'Active' : 'Inactive'}</span>
                          <label className="relative inline-flex items-center cursor-pointer">
                            <input
                              type="checkbox"
